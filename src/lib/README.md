...
- `db.ts`: [地位：持久化引擎] - [功能：提供基于 SQLite 的结构化查询封装引擎；`getBookByDataDir` 支持 URL 资源归属校验]
...
- annotations CRUD 在 `db.ts`：除基础标注/引用查询外，`sendAnnotationToNote` 在单个 SQLite transaction 中验证同书关系、按 note+annotation+section 幂等，并首次同时追加 Cornell 文本与插入 link；`importWereadAnnotations` 负责微信读书划线幂等映射。测试见 `annotations-db.test.ts`。
